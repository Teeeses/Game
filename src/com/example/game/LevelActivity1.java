...
public class LevelActivity1 extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		MainActivity.someOpenLevelList = 1;
	...
	{	
		...
		for(int i = 0; i < numberButtons; i++)
		{
			...
			{
				Drawable dr = getResources().getDrawable(R.drawable.box_button);
				Bitmap bitmap = ((BitmapDrawable) dr).getBitmap();
				int width = bitmap.getWidth();
				float scaleWidth = ((float) (screenWidth/6)) / width;
				Matrix matrix = new Matrix();
				matrix.postScale(scaleWidth, scaleWidth);
				...
				if(android.os.Build.VERSION.SDK_INT < 16) {
					arrayButtons[i].setBackgroundDrawable(draweble);
				}
				else {
					arrayButtons[i].setBackground(draweble);
				}
				...
				arrayButtons[i].setWidth(arrayButtons[i].getHeight());
			}
			else
			{
				Drawable dr = getResources().getDrawable(R.drawable.close_button);
				Bitmap bitmap = ((BitmapDrawable) dr).getBitmap();
				int width = bitmap.getWidth();
				float scaleWidth = ((float) (screenWidth/6)) / width;
				Matrix matrix = new Matrix();
